...
public class OptimizationFactory<T> {
	public LocalSearch<T, Double> hill_climbing(Variation_1_1<T> variation,
			HillClimbingReplacement<T> replace, Predicate<Solution<T>> tC) {
	...
	public LocalSearch<T, Double> simulated_annealing(
			...
			SimulatedAnnealingReplacement<T> replace, Predicate<Solution<T>> tC) {
		return new IterativeLocalSearch<T, Double>(
				new VariationReplaceLocalSearch<T>(variation, replace), tC);
	}

	public LocalSearch<T, Double> simulated_annealing(
			Variation_1_1<T> variation, SimulatedAnnealingScheme scheme,
			int MAX_ITERS) {
	...
	public LocalSearch<T, Double> simulated_annealing(
			Variation_1_1<T> variation, int K, int MAX_ITERS) {
		return simulated_annealing(variation,
				new SimpleSimulatedAnnealingScheme(K), MAX_ITERS);
	}
